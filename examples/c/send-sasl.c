...
static bool handle(app_data_t* app, pn_event_t* event) {
	 ...
	 pn_transport_t* pTransport = NULL;
     ...
     switch (pn_event_type(event)) {
   	...
   	case PN_CONNECTION_BOUND:
    	c = pn_event_connection(event);
 	 	pTransport = pn_event_transport(event);
     	pn_transport_require_auth(pTransport, true);
     	pn_transport_require_encryption(pTransport, false);
		pSasl = pn_sasl(pTransport);
    	pn_sasl_set_allow_insecure_mechs(pSasl, true);
		pn_sasl_allowed_mechs(pSasl, "PLAIN");
	    break;
...
int main(int argc, char **argv) {
  ...
  app.container_id = "my_container_id0";
  app.host = (argc > 1) ? argv[1] : "192.168.2.208";
  app.port = (argc > 2) ? argv[2] : "2009";
  app.amqp_address = (argc > 3) ? argv[3] : "waypoint.example";
  app.message_count = (argc > 4) ? atoi(argv[4]) : 10;
  app.user = (argc > 5) ? argv[5] : "tmpuser@ic1rh";
  app.password = (argc > 6) ? argv[6] : "tmppassword";
